...
class GetServer(commands.Cog):
    ...
    @commands.slash_command(description='Посмотреть инфо о сервере')
    async def server(self, interaction, server: int = commands.Param(name='сервер', description='От 1 до 29')):
        if server > 28:
            return await interaction.response.send_message('Укажите сервер не более 28!', ephemeral=True)
