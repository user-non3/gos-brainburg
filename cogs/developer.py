...
import zipfile
import psutil
from components.buttons.developer import DevButtons
...
class GetUser(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        if user is None:
            ...
            await interaction.response.send_message(embed=infoEmbed, ephemeral=True, view=DevButtons(user[2]))
...
class DeveloperPanel(commands.Cog):
    ...
    async def __dev(self, interaction):
        if interaction.author.id in access_list:
            ...
            await self.db.add_log(interaction.author, 1, 'изменил параметры Лидера [1, 2, 3]', datetime.now(), '/dev')
        else:
