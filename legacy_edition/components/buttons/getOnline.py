...
class GetOnline(disnake.ui.View):
    ...
    async def check_online(self, button: disnake.ui.Button, interaction: disnake.Interaction):
        try:
            if self.type == 1:
                r = requests.get(f"{api_url}/getLeader?token={api_token}&fraction={self.fraction_id}")
            
            elif self.type == 2:    
                r = requests.get(f"{api_url}/getDeputy?token={api_token}&fraction={self.fraction_id}")
                
            elif self.type == 3:
                r = requests.get(f"{api_url}/getMinister?token={api_token}&fraction={self.fraction_id}")
