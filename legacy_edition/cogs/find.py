...
class Find(commands.Cog):
    ...
    @commands.slash_command(name='find', description='Информация об игроке')
    async def __find(self, interaction, nick: str = commands.Param(name='никнейм', description='Формат Nick_Name')):
        ...
        try:
            ...
            findEmbed = disnake.Embed(
                ...
                description=f':gear: ID аккаунта: `{result.account_id}`\n:floppy_disk: Уровень: `{result.lvl}`\n\
            ...
            await add_log(self.bot, interaction.author, 'использовал команду', interaction.channel, f'/find NICK:{nick}')
