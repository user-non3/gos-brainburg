...
class checkMembers(commands.Cog):
    ...
    @commands.slash_command(name='members', description='Посмотреть онлайн фракции')
    async def __members(self, interaction, fraction: int = commands.Param(name='фракция', description='Введите ID фракции')):
        ...
        try:
            ...
            membersEmbed = disnake.Embed(
                title=f'Онлайн фракции {GetFractionNameByID[fraction]}',
            ...
            await add_log(self.bot, interaction.author, 'использовал команду', interaction.channel, f'/members FID:{fraction}')
