...
class GetLogs(commands.Cog):
    ...
    @commands.slash_command(name='logs', description='Посмотреть логи')
    async def __logs(self, interaction, 
                    nick: str = commands.Param(name='ник', description='Формат Nick_Name'),
                    limit: int = commands.Param(None, name='лимит', description='По умолчанию 15 строк')):
        ...
        if user[6] > 2 or user[6] == 2:
            ...
            try:
                ...
                request = requests.get(f"{api_url}/getLogs?token={api_token}&leader={nick}&limit={limit}")
                data = request.json()
                ...
                logger.error(f'[getLogs][CMD] [type:GET] {ex}')
