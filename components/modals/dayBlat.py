from utils import *
from utils.fraction import GetFractionID, GetFractionList
...
class ManageModal(disnake.ui.Modal):
    def __init__(self, type, fraction):
        ...
        if self.type == 2:
            components = [
                disnake.ui.TextInput(label="Введите ник игрока", placeholder='Пример Leonid_Stealer', custom_id="player_nick", required=True)
            ]
        else:
            components = [
                ...
                disnake.ui.TextInput(label="Введите ник игрока", placeholder='Пример Violetta_Wallace', custom_id="player_nick", required=True),
                disnake.ui.TextInput(label="Введите фракцию", placeholder='Пример LVMC, LSMC', custom_id="fraction", required=True),
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        if self.type == 1:
            ...
            await self.db.create_ranks_log(leader_nick, player_nick, GetFractionID(fraction), rank, date[0], date[1], 'День блата', self.fraction)
            await interaction.response.send_message(f'Вы успешно создали запись с ником `{player_nick}`! :white_check_mark:', ephemeral=True)
            logger.info(f'[RANK_LOGS] Пользователь {interaction.author.name} [{interaction.author.id}] создал запись с параметрами: {leader_nick} | {player_nick}\
                        | {GetFractionID(fraction)} | {rank} | {date[0]} {date[1]}')

        elif self.type == 2:
            player_nick = interaction.text_values["player_nick"]
            logs_list_get = await self.db.get_ranks_logs_list(self.fraction)
            print(logs_list_get[0][2])
            if player_nick == logs_list_get[0][2]:
                ...
                logger.info(f'[RANK_LOGS] Пользователь {interaction.author.name} [{interaction.author.id}] удалил запись с ником `{player_nick}`')
